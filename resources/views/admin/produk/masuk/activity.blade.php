@forelse ($recentTransaksi as $mas)
    <tr class="hover:bg-muted/30 transition">
        <!-- WAKTU -->
        <td class="p-4 pl-6 text-foreground">
            <div class="flex flex-col leading-tight">
                <span class="font-medium">
                    {{ $mas->created_at->format('d M Y') }}
                </span>
                <span class="text-xs text-secondary">
                    {{ $mas->created_at->format('H:i') }}
                </span>
            </div>
        </td>
        <!-- USER -->
        <td class="p-4">
            <div class="flex items-center gap-3">
                <div
                    ...
                    @if ($mas->creator && $mas->creator->dataDiri && $mas->creator->dataDiri->foto_diri)
                        <img src="{{ asset('foto_profile/' . $mas->creator->dataDiri->foto_diri) }}"
                            class="size-9 rounded-full object-cover">
                    @else
                        {{ $mas->creator ? strtoupper(substr($mas->creator->username, 0, 2)) : 'AD' }}
                    @endif
                </div>
                <div class="flex flex-col leading-tight">
                    <span class="font-medium text-foreground">
                        {{ $mas->creator->username }}
                    </span>
                </div>
            </div>
        </td>
        <!-- AKTIVITAS -->
        <td class="p-4">
            <span class="text-secondary">Memasukkan</span>
            <span class="font-semibold text-foreground">
                {{ $mas->jumlah }}
            </span>
            <span class="text-secondary">produk</span>
            <span class="font-medium text-primary">
                {{ $mas->produk->nama_produk }}
            </span>
        </td>
        <!-- STATUS -->
        <td class="p-4">
            @if ($mas->status == 'posted')
                <span
        ...
        <td colspan="4" class="p-10 text-center">
            <div class="flex flex-col items-center justify-center gap-3 text-secondary">
                ...
                <div class="text-sm">
                    ...
                    <p class="text-xs">Transaksi stok masuk akan muncul di sini</p>
                </div>
            </div>
        </td>
    </tr>
