<x-view.layout.app title="Kartu Stok">
    ...
    <div id="view-order-list" class="view-section hidden flex flex-col flex-1 h-full">
        <div class="flex items-center gap-2 mb-6 text-sm text-secondary">
            <a href="{{ route('user.stok.masuk') }}" onclick="switchView('order-masuk')"
                class="hover:text-primary transition-colors">Stok Masuk</a>
            <i data-lucide="chevron-right" class="size-4"></i>
            <a href="{{ route('user.stok.keluar') }}" onclick="switchView('order-keluar')"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-border" id="table-kartu">
                            @include('user.stok.kartustok.table')
                        </tbody>
                    </table>
                </div>
        ...
        <script>
            ...
            if (searchInput) {
                ...
                searchInput.addEventListener('keyup', function() {
                    ...
                    timer = setTimeout(() => {
                        ...
                        fetch(`{{ route('user.kartu') }}?search=${keyword}`, {
                                headers: {
            ...
            async function openDetailKartu(id) {
                ...
                try {
                    ...
                    const res = await fetch(`/user/menu/kartu-detail/${id}`);
                    ...
                    document.getElementById('detail-dskr').innerText =
                        produk.keterangan ?? '-';
