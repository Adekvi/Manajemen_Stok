        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[700px] text-sm">
                ...
                <tbody class="divide-y divide-border" id="activityTable" data-url="{{ url()->current() }}">
                    @include('user.dashboard.activity-table')
                </tbody>
            </table>
        </div>
    </div>
        ...
        <script>
            ...
            function initializeCharts() {
                ...
                if (attEl) {
                    ...
                    const safeData = attendanceData.length ? attendanceData : new Array(7).fill(0);

                    new Chart(attEl, {
                        type: 'line',
                        data: {
                            labels: attendanceLabels.length ? attendanceLabels : ['-', '-', '-', '-', '-', '-', '-'],
                            datasets: [{
                                label: 'Transaksi',
                                data: safeData,
                                borderColor: '#165DFF',
